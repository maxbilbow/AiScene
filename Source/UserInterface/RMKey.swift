...
class RMKey : CustomStringConvertible{
    ...
    var actionProcessor: RMSActionProcessor {
    ...
    func press(object: Any? = nil) -> Bool{
        if self.isRepeating {
            ...
            self.isPressed = true
            if asString {
                return self.actionProcessor.action(self.string, speed: self.speed.on, args: object ?? self.values)
            } else {
                return self.actionProcessor.action(self.action, speed: self.speed.on, args: object ?? self.values)
            }
        }
    }
    ...
    func release(object: Any? = nil) -> Bool{
        if self.isPressed {
            ...
            if asString {
                return self.actionProcessor.action(self.string, speed: self.speed.off, args: self.values)
            } else {
                return self.actionProcessor.action(self.action, speed: self.speed.off, args: self.values)
            }
...
//        NSLog(self.print)
        if self.isRepeating && self.isPressed {
            if asString {
                ...
                self.actionProcessor.action(self.action, speed: self.speed.on, args: self.values)
            }
