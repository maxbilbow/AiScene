...
class RMXArt {
    ...
    class func drawAxis(world: RMXScene, radius: RMFloat) {//xCol y:(float*)yCol z:(float*)zCol{
        ...
        func drawAxis(axis: String) {
            ...
            let sprite = RMXSprite(inWorld: world, geometryNode: node, type: RMXSpriteType.BACKGROUND, shape: .CUBE, unique: true)
    ...
    //BOOL gravity = true;
        ...
        for(var i: RMFloat = -noOfShapes / 2; i < noOfShapes / 2; ++i) {
            ...
            let sprite = RMXSprite(inWorld: world, geometryNode: node, type: .PASSIVE, shape: shape, unique: false)
